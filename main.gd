...
# 松手：按当前位置 + 甩动速度决定落到哪一页，用缓动补间平滑吸附（丝滑）。
func _gal_settle() -> void:
	if _gal_tw != null and _gal_tw.is_valid():
		_gal_tw.kill()
	var n := MODELS.size()
	if n <= 1:
		return
	# 甩动越快，越倾向多翻一页（bias 限制在 ±0.5 内，最多落到相邻页）。
	var bias: float = clampf(_gal_vel * 0.09, -0.5, 0.5)
	var slot: float = round(_gal_scroll / PAGE_DX + bias)
	var target: float = slot * PAGE_DX
	# 时长随距离自适应，短则快、长则不拖沓。
	var dur: float = clampf(absf(target - _gal_scroll) / PAGE_DX * 0.42 + 0.12, 0.16, 0.55)
	_gal_tw = create_tween().set_trans(Tween.TRANS_CUBIC).set_ease(Tween.EASE_OUT)
	_gal_tw.tween_property(self, "_gal_scroll", target, dur)
